...
// Low energy classification ranges
const double LOW_ENERGY_RANGE1_MIN = 10.0;
const double LOW_ENERGY_RANGE1_MAX = 25.0;
const double LOW_ENERGY_RANGE2_MIN = 30.0;
const double LOW_ENERGY_RANGE2_MAX = 44.0;
...
void saveAllHistogramsToRootFile(TFile* rootFile, 
                                ...
                                TH1D* h_dt_low_muon_range1, TH1D* h_dt_low_muon_range2,
                                TH1D* h_energy_range1, TH1D* h_energy_range2) {
    ...
    if (h_dt_low_muon_range1) h_dt_low_muon_range1->Write();
    if (h_dt_low_muon_range2) h_dt_low_muon_range2->Write();
    if (h_energy_range1) h_energy_range1->Write();
    if (h_energy_range2) h_energy_range2->Write();
...
int main(int argc, char *argv[]) {
    ...
    // New histograms for low energy classification
    TH1D* h_dt_low_muon_range1 = new TH1D("dt_low_muon_range1", 
        "#Delta t Low Energy Isolated (10-25 p.e.) to Muon;#Delta t [#mus];Counts/10#mus", 
        120, 0, 1200);
    TH1D* h_dt_low_muon_range2 = new TH1D("dt_low_muon_range2", 
        "#Delta t Low Energy Isolated (30-44 p.e.) to Muon;#Delta t [#mus];Counts/10#mus", 
        120, 0, 1200);
    
    // New histograms for energy spectrum of both ranges
    TH1D* h_energy_range1 = new TH1D("energy_range1", 
        "Energy Spectrum (10-25 p.e.);Energy (p.e.);Counts", 
        50, 0, 50);
    TH1D* h_energy_range2 = new TH1D("energy_range2", 
        "Energy Spectrum (30-44 p.e.);Energy (p.e.);Counts", 
        50, 0, 50);
        ...
        for (int iEnt = 0; iEnt < numEntries; iEnt++) {
            ...
            if (is_isolated) {
                ...
                if (p.energy <= 100 && p.number >= 4) {
                    ...
                    // Fill classified low energy histograms ONLY for dt_muon >= 16 μs
                    if (dt_muon >= LOW_ENERGY_DT_MIN) {
                        if (p.energy >= LOW_ENERGY_RANGE1_MIN && p.energy <= LOW_ENERGY_RANGE1_MAX) {
                            h_dt_low_muon_range1->Fill(dt_muon);
                            h_energy_range1->Fill(p.energy);
                        } else if (p.energy >= LOW_ENERGY_RANGE2_MIN && p.energy <= LOW_ENERGY_RANGE2_MAX) {
                            h_dt_low_muon_range2->Fill(dt_muon);
                            h_energy_range2->Fill(p.energy);
                        }
                    }
    ...
    // Create the new classified low energy plot with custom stat boxes
    TCanvas *c_low_muon_classified = new TCanvas("c_low_muon_classified", "DeltaT Low to Muon - Classified by Energy", 1200, 800);
    c_low_muon_classified->SetLeftMargin(0.15);
    c_low_muon_classified->SetRightMargin(0.08);
    c_low_muon_classified->SetBottomMargin(0.12);
    c_low_muon_classified->SetTopMargin(0.08);

    // Turn off default stats
    gStyle->SetOptStat(0);
    gStyle->SetOptFit(0);

    // Style both histograms IDENTICAL to the original plot
    h_dt_low_muon_range1->SetLineWidth(2);
    h_dt_low_muon_range1->SetLineColor(kRed);
    h_dt_low_muon_range1->SetFillColor(kRed);
    h_dt_low_muon_range1->SetFillStyle(3001);
    
    h_dt_low_muon_range2->SetLineWidth(2);
    h_dt_low_muon_range2->SetLineColor(kBlue);
    h_dt_low_muon_range2->SetFillColor(kBlue);
    h_dt_low_muon_range2->SetFillStyle(3001);

    // Set axis properties IDENTICAL to original plot
    h_dt_low_muon_range1->GetXaxis()->SetTitleSize(0.04);
    h_dt_low_muon_range1->GetYaxis()->SetTitleSize(0.05);
    h_dt_low_muon_range1->GetXaxis()->SetLabelSize(0.04);
    h_dt_low_muon_range1->GetYaxis()->SetLabelSize(0.04);
    h_dt_low_muon_range1->SetTitle("#Delta t: Low Energy Isolated to Muon (Classified by Energy)");
    h_dt_low_muon_range1->GetXaxis()->SetTitle("#Delta t [#mus]");
    h_dt_low_muon_range1->GetYaxis()->SetTitle("Counts/10#mus");

    // Calculate proper Y-axis maximum based on the data (only data >= 16 μs will be there)
    double max_range1 = h_dt_low_muon_range1->GetMaximum();
    double max_range2 = h_dt_low_muon_range2->GetMaximum();
    double data_max = TMath::Max(max_range1, max_range2);
    h_dt_low_muon_range1->SetMaximum(data_max * 1.15); // 15% margin for better visibility

    // Draw range1 first, then range2 - same drawing style
    h_dt_low_muon_range1->Draw("HIST");
    h_dt_low_muon_range2->Draw("HIST SAME");

    // Perform fits for both ranges using IDENTICAL fit parameters and ranges
    TF1* expFit_range1 = nullptr;
    TF1* expFit_range2 = nullptr;
    
    double N0_range1 = 0, tau_range1 = 0, C_range1 = 0;
    double N0_range2 = 0, tau_range2 = 0, C_range2 = 0;
    double chi2_ndf_range1 = 0, chi2_ndf_range2 = 0;
    
    // Fit for range 1 (10-25 p.e.) - IDENTICAL to original fit procedure
    if (h_dt_low_muon_range1->GetEntries() > 10) {
        expFit_range1 = new TF1("expFit_range1", ExpFit, FIT_MIN_LOW_MUON, FIT_MAX_LOW_MUON, 3);
        expFit_range1->SetParNames("N_{0}", "#tau", "C");

        double integral1 = h_dt_low_muon_range1->Integral(
            h_dt_low_muon_range1->FindBin(FIT_MIN_LOW_MUON), 
            h_dt_low_muon_range1->FindBin(FIT_MAX_LOW_MUON)
        );
        double bin_width1 = h_dt_low_muon_range1->GetBinWidth(1);
        double C_init1 = 0;
        int bin_400_1 = h_dt_low_muon_range1->FindBin(400.0);
        int bin_500_1 = h_dt_low_muon_range1->FindBin(500.0);
        double min_content1 = 1e9;
        for (int i = bin_400_1; i <= bin_500_1; i++) {
            double content = h_dt_low_muon_range1->GetBinContent(i);
            if (content > 0 && content < min_content1) min_content1 = content;
        }
        if (min_content1 < 1e9) C_init1 = min_content1;
        else C_init1 = 0.1;

        double N0_init1 = (integral1 * bin_width1 - C_init1 * (FIT_MAX_LOW_MUON - FIT_MIN_LOW_MUON)) / 200.0;
        if (N0_init1 <= 0) N0_init1 = 1.0;

        expFit_range1->SetParameters(N0_init1, 200.0, C_init1);
        expFit_range1->SetParLimits(0, 0, N0_init1 * 100);
        expFit_range1->SetParLimits(1, 0.1, 1000.0);
        expFit_range1->SetParLimits(2, -C_init1 * 10, C_init1 * 10);

        int fitStatus1 = h_dt_low_muon_range1->Fit(expFit_range1, "QRN+", "", FIT_MIN_LOW_MUON, FIT_MAX_LOW_MUON);

        expFit_range1->SetLineColor(kRed);
        expFit_range1->SetLineWidth(3);
        expFit_range1->Draw("SAME");

        N0_range1 = expFit_range1->GetParameter(0);
        tau_range1 = expFit_range1->GetParameter(1);
        C_range1 = expFit_range1->GetParameter(2);
        chi2_ndf_range1 = expFit_range1->GetChisquare() / expFit_range1->GetNDF();

        cout << "=== Fit Results for Low Energy Range 1 (10-25 p.e.) ===" << endl;
        cout << "Fit Status: " << fitStatus1 << " (0 = success)" << endl;
        cout << Form("N₀ = %.1f ± %.1f", N0_range1, expFit_range1->GetParError(0)) << endl;
        cout << Form("τ = %.1f ± %.1f µs", tau_range1, expFit_range1->GetParError(1)) << endl;
        cout << Form("C = %.1f ± %.1f", C_range1, expFit_range1->GetParError(2)) << endl;
        cout << Form("χ²/NDF = %.2f", chi2_ndf_range1) << endl;
        cout << "Events in range: " << h_dt_low_muon_range1->GetEntries() << endl;
        cout << "======================================================" << endl;
    }

    // Fit for range 2 (30-44 p.e.) - IDENTICAL to original fit procedure
    if (h_dt_low_muon_range2->GetEntries() > 10) {
        expFit_range2 = new TF1("expFit_range2", ExpFit, FIT_MIN_LOW_MUON, FIT_MAX_LOW_MUON, 3);
        expFit_range2->SetParNames("N_{0}", "#tau", "C");

        double integral2 = h_dt_low_muon_range2->Integral(
            h_dt_low_muon_range2->FindBin(FIT_MIN_LOW_MUON), 
            h_dt_low_muon_range2->FindBin(FIT_MAX_LOW_MUON)
        );
        double bin_width2 = h_dt_low_muon_range2->GetBinWidth(1);
        double C_init2 = 0;
        int bin_400_2 = h_dt_low_muon_range2->FindBin(400.0);
        int bin_500_2 = h_dt_low_muon_range2->FindBin(500.0);
        double min_content2 = 1e9;
        for (int i = bin_400_2; i <= bin_500_2; i++) {
            double content = h_dt_low_muon_range2->GetBinContent(i);
            if (content > 0 && content < min_content2) min_content2 = content;
        }
        if (min_content2 < 1e9) C_init2 = min_content2;
        else C_init2 = 0.1;

        double N0_init2 = (integral2 * bin_width2 - C_init2 * (FIT_MAX_LOW_MUON - FIT_MIN_LOW_MUON)) / 200.0;
        if (N0_init2 <= 0) N0_init2 = 1.0;

        expFit_range2->SetParameters(N0_init2, 200.0, C_init2);
        expFit_range2->SetParLimits(0, 0, N0_init2 * 100);
        expFit_range2->SetParLimits(1, 0.1, 1000.0);
        expFit_range2->SetParLimits(2, -C_init2 * 10, C_init2 * 10);

        int fitStatus2 = h_dt_low_muon_range2->Fit(expFit_range2, "QRN+", "", FIT_MIN_LOW_MUON, FIT_MAX_LOW_MUON);

        expFit_range2->SetLineColor(kBlue);
        expFit_range2->SetLineWidth(3);
        expFit_range2->Draw("SAME");

        N0_range2 = expFit_range2->GetParameter(0);
        tau_range2 = expFit_range2->GetParameter(1);
        C_range2 = expFit_range2->GetParameter(2);
        chi2_ndf_range2 = expFit_range2->GetChisquare() / expFit_range2->GetNDF();

        cout << "=== Fit Results for Low Energy Range 2 (30-44 p.e.) ===" << endl;
        cout << "Fit Status: " << fitStatus2 << " (0 = success)" << endl;
        cout << Form("N₀ = %.1f ± %.1f", N0_range2, expFit_range2->GetParError(0)) << endl;
        cout << Form("τ = %.1f ± %.1f µs", tau_range2, expFit_range2->GetParError(1)) << endl;
        cout << Form("C = %.1f ± %.1f", C_range2, expFit_range2->GetParError(2)) << endl;
        cout << Form("χ²/NDF = %.2f", chi2_ndf_range2) << endl;
        cout << "Events in range: " << h_dt_low_muon_range2->GetEntries() << endl;
        cout << "======================================================" << endl;
    }
// Create custom stat boxes for both ranges
TPaveText *stat1 = new TPaveText(0.65, 0.80, 0.85, 0.90, "NDC");
stat1->SetFillColor(0);
stat1->SetBorderSize(1);
stat1->SetTextColor(kRed);
stat1->SetTextSize(0.03);
stat1->SetTextAlign(12);
stat1->AddText(Form("10-25 p.e.: %d events", (int)h_dt_low_muon_range1->GetEntries()));
if (expFit_range1) {
    stat1->AddText(Form("#chi^{2}/ndf = %.2f", chi2_ndf_range1));
    double tau1_err = expFit_range1->GetParError(1);
    stat1->AddText(Form("#tau = %.1f #pm %.1f #mus", tau_range1, tau1_err));
}
stat1->Draw();

TPaveText *stat2 = new TPaveText(0.65, 0.65, 0.85, 0.75, "NDC");
stat2->SetFillColor(0);
stat2->SetBorderSize(1);
stat2->SetTextColor(kBlue);
stat2->SetTextSize(0.03);
stat2->SetTextAlign(12);
stat2->AddText(Form("30-44 p.e.: %d events", (int)h_dt_low_muon_range2->GetEntries()));
if (expFit_range2) {
    stat2->AddText(Form("#chi^{2}/ndf = %.2f", chi2_ndf_range2));
    double tau2_err = expFit_range2->GetParError(1);
    stat2->AddText(Form("#tau = %.1f #pm %.1f #mus", tau_range2, tau2_err));
}
stat2->Draw();
    c_low_muon_classified->Update();
    plotName = OUTPUT_DIR + "/DeltaT_Low_to_Muon_Classified.png";
    c_low_muon_classified->SaveAs(plotName.c_str());
    cout << "Saved classified low energy plot: " << plotName << endl;

    // Clean up
    if (expFit_range1) delete expFit_range1;
    if (expFit_range2) delete expFit_range2;
    delete stat1;
    delete stat2;
    delete c_low_muon_classified;

// Turn off default stats BEFORE creating the canvas
gStyle->SetOptStat(0);
gStyle->SetOptFit(0);
    // Create the energy spectrum plot for both ranges
    TCanvas *c_energy_spectrum = new TCanvas("c_energy_spectrum", "Energy Spectrum - Classified Ranges", 1200, 800);
    c_energy_spectrum->SetLeftMargin(0.15);
    c_energy_spectrum->SetRightMargin(0.08);
    c_energy_spectrum->SetBottomMargin(0.12);
    c_energy_spectrum->SetTopMargin(0.08);

    // Style energy histograms
    h_energy_range1->SetLineWidth(2);
    h_energy_range1->SetLineColor(kRed);
    h_energy_range1->SetFillColor(kRed);
    h_energy_range1->SetFillStyle(3001);
    
    h_energy_range2->SetLineWidth(2);
    h_energy_range2->SetLineColor(kBlue);
    h_energy_range2->SetFillColor(kBlue);
    h_energy_range2->SetFillStyle(3001);

    // Set axis properties
    h_energy_range1->GetXaxis()->SetTitleSize(0.04);
    h_energy_range1->GetYaxis()->SetTitleSize(0.05);
    h_energy_range1->GetXaxis()->SetLabelSize(0.04);
    h_energy_range1->GetYaxis()->SetLabelSize(0.04);
    h_energy_range1->SetTitle("Energy Spectrum of Classified Low Energy Ranges");
    h_energy_range1->GetXaxis()->SetTitle("Energy (p.e.)");
    h_energy_range1->GetYaxis()->SetTitle("Counts");

    // Calculate proper Y-axis maximum
    double max_energy1 = h_energy_range1->GetMaximum();
    double max_energy2 = h_energy_range2->GetMaximum();
    double energy_max = TMath::Max(max_energy1, max_energy2);
    h_energy_range1->SetMaximum(energy_max * 1.15);

    // Draw both histograms
    h_energy_range1->Draw("HIST");
    h_energy_range2->Draw("HIST SAME");

    // Add legend for energy spectrum - placed BELOW the stat boxes to prevent overlapping
    // Add legend for energy spectrum - moved to top left corner
TLegend *leg_energy = new TLegend(0.65, 0.75, 0.85, 0.90); // Moved to bottom left
    leg_energy->SetBorderSize(0);
    leg_energy->SetFillStyle(0);
    leg_energy->SetTextSize(0.035);
    leg_energy->AddEntry(h_energy_range1, Form("10-25 p.e. (%d events)", (int)h_energy_range1->GetEntries()), "f");
    leg_energy->AddEntry(h_energy_range2, Form("30-44 p.e. (%d events)", (int)h_energy_range2->GetEntries()), "f");
    leg_energy->Draw();

    c_energy_spectrum->Update();
    plotName = OUTPUT_DIR + "/Energy_Spectrum_Classified.png";
    c_energy_spectrum->SaveAs(plotName.c_str());
    cout << "Saved energy spectrum plot: " << plotName << endl;

    delete leg_energy;
    delete c_energy_spectrum;

    gStyle->SetOptStat(1111);
gStyle->SetOptFit(1111);
    // Continue with the original low muon plot (unchanged)
    ...
    TLegend *leg_michel = new TLegend(0.7, 0.7, 0.9, 0.9);
    leg_michel->SetBorderSize(0);
    leg_michel->SetFillStyle(0);
    leg_michel->SetTextSize(0.035);
    leg_michel->AddEntry(h_michel_energy_fit_range, "Michel Energy (2-16 μs)", "l");
    leg_michel->AddEntry(h_michel_energy_predicted, "Predicted Michel Energy (16-100 μs)", "l");
    leg_michel->Draw();

    c_michel_method->Update();
    plotName = OUTPUT_DIR + "/Michel_Background_Method.png";
    c_michel_method->SaveAs(plotName.c_str());
    cout << "Saved Michel background method plot: " << plotName << endl;
    ...
    // Create normalized energy spectrum plot (normalized to 30 days)
    TCanvas *c_energy_spectrum_norm = new TCanvas("c_energy_spectrum_norm", "Energy Spectrum - Classified Ranges (Normalized to 30 Days)", 1200, 800);
    c_energy_spectrum_norm->SetLeftMargin(0.15);
    c_energy_spectrum_norm->SetRightMargin(0.08);
    c_energy_spectrum_norm->SetBottomMargin(0.12);
    c_energy_spectrum_norm->SetTopMargin(0.08);

    // Create normalized copies of the histograms
    TH1D* h_energy_range1_norm = (TH1D*)h_energy_range1->Clone("energy_range1_norm");
    TH1D* h_energy_range2_norm = (TH1D*)h_energy_range2->Clone("energy_range2_norm");
    
    // Normalize to 30 days
    double norm_factor_30days = 30.0 / liveTimeDays;
    h_energy_range1_norm->Scale(norm_factor_30days);
    h_energy_range2_norm->Scale(norm_factor_30days);

    // Style normalized histograms
    h_energy_range1_norm->SetLineWidth(2);
    h_energy_range1_norm->SetLineColor(kRed);
    h_energy_range1_norm->SetFillColor(kRed);
    h_energy_range1_norm->SetFillStyle(3001);
    
    h_energy_range2_norm->SetLineWidth(2);
    h_energy_range2_norm->SetLineColor(kBlue);
    h_energy_range2_norm->SetFillColor(kBlue);
    h_energy_range2_norm->SetFillStyle(3001);

    // Set axis properties
    h_energy_range1_norm->GetXaxis()->SetTitleSize(0.04);
    h_energy_range1_norm->GetYaxis()->SetTitleSize(0.05);
    h_energy_range1_norm->GetXaxis()->SetLabelSize(0.04);
    h_energy_range1_norm->GetYaxis()->SetLabelSize(0.04);
    h_energy_range1_norm->SetTitle("Energy Spectrum of Classified Low Energy Ranges (Normalized to 30 Days)");
    h_energy_range1_norm->GetXaxis()->SetTitle("Energy (p.e.)");
    h_energy_range1_norm->GetYaxis()->SetTitle("Counts per 30 Days");

    // Calculate proper Y-axis maximum for normalized plot
    double max_energy1_norm = h_energy_range1_norm->GetMaximum();
    double max_energy2_norm = h_energy_range2_norm->GetMaximum();
    double energy_max_norm = TMath::Max(max_energy1_norm, max_energy2_norm);
    h_energy_range1_norm->SetMaximum(energy_max_norm * 1.15);

    // Draw both normalized histograms
    h_energy_range1_norm->Draw("HIST");
    h_energy_range2_norm->Draw("HIST SAME");

    // Add legend for normalized energy spectrum - placed at bottom to prevent overlapping
    TLegend *leg_energy_norm = new TLegend(0.15, 0.15, 0.45, 0.30);
    leg_energy_norm->SetBorderSize(0);
    leg_energy_norm->SetFillStyle(0);
    leg_energy_norm->SetTextSize(0.035);
    leg_energy_norm->AddEntry(h_energy_range1_norm, Form("10-25 p.e. (%.0f events/30d)", h_energy_range1_norm->Integral()), "f");
    leg_energy_norm->AddEntry(h_energy_range2_norm, Form("30-44 p.e. (%.0f events/30d)", h_energy_range2_norm->Integral()), "f");
    leg_energy_norm->AddEntry((TObject*)0, Form("Actual live time: %.2f days", liveTimeDays), "");
    leg_energy_norm->Draw();

    c_energy_spectrum_norm->Update();
    plotName = OUTPUT_DIR + "/Energy_Spectrum_Classified_Normalized_30days.png";
    c_energy_spectrum_norm->SaveAs(plotName.c_str());
    cout << "Saved normalized energy spectrum plot: " << plotName << endl;

    // Clean up
    delete leg_energy_norm;
    delete c_energy_spectrum_norm;
    delete h_energy_range1_norm;
    delete h_energy_range2_norm;
    ...
    saveAllHistogramsToRootFile(rootFile, 
                               ...
                               h_dt_low_muon_range1, h_dt_low_muon_range2,
                               h_energy_range1, h_energy_range2);
    // Close ROOT file
    rootFile->Close();
    ...
    delete h_dt_low_muon_range1;
    delete h_dt_low_muon_range2;
    delete h_energy_range1;
    delete h_energy_range2;
    delete leg_sub1;
    delete leg_sub2;
    delete leg_sub2_log;
