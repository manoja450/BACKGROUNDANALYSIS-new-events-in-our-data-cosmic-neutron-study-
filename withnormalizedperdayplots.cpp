...
void saveLiveTimeInfo(double totalLiveTime, const string& outputDir) {
...
int main(int argc, char *argv[]) {
    ...
    // Save live time information
    saveLiveTimeInfo(totalLiveTime, OUTPUT_DIR);
    
    cout << "=== Live Time Summary ===" << endl;
    cout << "Total Live Time: " << totalLiveTime << " seconds" << endl;
    cout << "Total Live Time: " << totalLiveTime / 60.0 << " minutes" << endl;
    cout << "Total Live Time: " << totalLiveTime / 3600.0 << " hours" << endl;
    cout << "Total Live Time: " << liveTimeDays << " days" << endl;
    cout << "=========================" << endl;
    ...
    if (h_dt_michel_sideband->GetEntries() > 20) {
        ...
        if (fit_status == 0) {
            ...
            if (tau_fit > 0) {
                ...
                cout << "Fit range: " << MICHEL_FIT_RANGE_MIN << "-" << MICHEL_FIT_RANGE_MAX << " μs" << endl;
                cout << "Prediction range: " << MICHEL_PREDICTION_MIN << "-" << MICHEL_PREDICTION_MAX << " μs" << endl;
                cout << "Time bin size: " << TIME_BIN_SIZE << " μs" << endl;
                
                int bin_count = 0;
                for (int i = 0; i < 840; i++) { // 84.0 μs / 0.1 μs = 840 bins
                    ...
                    if (i < 5 || i % 100 == 0 || i >= 835) {
                        cout << Form("  Bin %3d: %5.1f-%5.1f μs (center=%5.2f μs), N(t)=%6.3f", 
                                    i, t_start, t_end, t_center, N_t) << endl;
                    }
                }
                
                predicted_michels_err = sqrt(predicted_michels_err);
                
                cout << Form("Total predicted Michel events: %.1f ± %.1f", predicted_michels, predicted_michels_err) << endl;
                cout << "Number of bins: " << bin_count << endl;
                
                // Verify the calculation
                cout << "Verification: First bin N(" << MICHEL_PREDICTION_MIN + 0.05 << ") = " 
                     << (N0_fit * exp(-(MICHEL_PREDICTION_MIN + 0.05) / tau_fit)) << endl;
                cout << "Verification: Last bin N(99.95) = " 
                     << (N0_fit * exp(-99.95 / tau_fit)) << endl;
            }
        } else {
    ...
    // Scale the energy spectrum from fit range to prediction range
    TH1D* h_michel_energy_predicted = (TH1D*)h_michel_energy_fit_range->Clone("michel_energy_predicted");
    double fit_range_integral = h_dt_michel_sideband->Integral(
        h_dt_michel_sideband->FindBin(MICHEL_FIT_RANGE_MIN),
        h_dt_michel_sideband->FindBin(MICHEL_FIT_RANGE_MAX)
    );

    double michel_scale = (predicted_michels > 0 && fit_range_integral > 0) ? predicted_michels / fit_range_integral : 0;
    ...
    // Plot 2: Energy spectrum from fit range only
    ...
    cout << "Analysis complete. Results saved in " << OUTPUT_DIR << "/ (*.png, *.csv, *.txt)" << endl;
    return 0;
}
